...
export default function VillageLeadershipPanel({ state }: { state: WorldState }) {
  ...
  return (
    <div className="rounded-xl border border-amber-600/30 bg-amber-950/20 p-3">
      ...
      {ceremonyStatus && (
        <p className="mb-2 rounded-lg bg-amber-900/30 px-2 py-1.5 text-[11px] leading-relaxed text-amber-100/90">
          {ceremonyStatus}
        </p>
      )}
      ...
      {leader ? (
        ...
        <p className="mb-2 text-[11px] text-rose-300">No village head — adults will elect when eligible.</p>
      )}

      {rankings.length > 1 && (
        <div className="space-y-1">
          <p className="text-[11px] font-semibold text-stone-400">Leadership standings</p>
          {rankings.map((c, i) => (
            <div
              key={c.entityId}
